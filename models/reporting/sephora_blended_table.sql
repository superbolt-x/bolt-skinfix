{{ config (
  ...
  (SELECT 'Facebook' as channel, date, date_granularity, market, product_name,
    ...
    GROUP BY channel, date, date_granularity, market, product_name)
    
    UNION ALL
    
    (SELECT 'TikTok' as channel, date, date_granularity, market, product_name,
    ...
    (SELECT 'Youtube Shorts' as channel, date, date_granularity, market, product_name,
        COALESCE(SUM(spend),0) AS spend, COALESCE(SUM(impressions),0) AS impressions, COALESCE(SUM(clicks),0) AS clicks, COALESCE(SUM(purchases),0) AS purchases, 
        COALESCE(SUM(revenue),0) AS revenue
