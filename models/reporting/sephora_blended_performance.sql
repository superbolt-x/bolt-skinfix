{{ config (
  ...
  (SELECT 'Facebook' as channel, date, date_granularity, market, campaign_name, adset_name as ad_group_name, ad_name, product_name,
    ...
    GROUP BY channel, date, date_granularity, market, campaign_name, ad_group_name, ad_name, product_name)
    
    UNION ALL
    
    (SELECT 'TikTok' as channel, date, date_granularity, market, campaign_name, adgroup_name as ad_group_name, ad_name, product_name,
    ...
    (SELECT 'Youtube Shorts' as channel, date, date_granularity, market, campaign_name, ad_group_name, ad_name, product_name,
        COALESCE(SUM(spend),0) AS spend, COALESCE(SUM(impressions),0) AS impressions, COALESCE(SUM(clicks),0) AS clicks, COALESCE(SUM(purchases),0) AS purchases, 
        COALESCE(SUM(revenue),0) AS revenue
